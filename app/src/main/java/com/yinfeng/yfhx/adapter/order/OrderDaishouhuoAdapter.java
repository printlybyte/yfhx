 ...
 * ============================================
 **/
public class OrderDaishouhuoAdapter extends BaseQuickAdapter<OrderActivityBean.DataBean.ListBean, OrderDaishouhuoAdapter.MyHolder> {
    private Context context = Latte.getApplicationContext();

    public OrderDaishouhuoAdapter(int layoutResId, List data) {
    ...
    protected void convert(@NonNull MyHolder helper, OrderActivityBean.DataBean.ListBean item) {
        ...
        helper.setText(R.id.ri_daishouhuo_item_order_sn, item.getOrder_sn() == "" ? tipStr : "订单号：" + item.getOrder_sn());
        helper.setText(R.id.ri_daishouhuo_item_order_time, item.getOrder_time() == "" ? tipStr : "下单时间：" + item.getOrder_time());
        helper.setText(R.id.ri_daishouhuo_item_shop_name, item.getShop_name() == "" ? tipStr :  item.getShop_name());
        helper.setText(R.id.ri_daishouhuo_item_order_status, item.getOrder_status() == "" ? tipStr :  item.getOrder_status());
        helper.setText(R.id.ri_daishouhuo_item_total_price, item.getTotal_amount_formated() == "" ? tipStr : "" + item.getTotal_amount_formated());
        helper.setText(R.id.ri_daishouhuo_item_total_nums, item.getOrder_goods().size() + "" == "" ? tipStr : "共" + item.getOrder_goods().size() + "件商品：");

        helper.addOnClickListener(R.id.ri_daishouhuo_item_group);
//        helper.addOnClickListener(R.id.ri_daishouhuo_item_request);
//        helper.addOnClickListener(R.id.ri_daishouhuo_item_confirm);
//        helper.addOnClickListener(R.id.ri_daishouhuo_item_query_order);
        ...
        if (item.getOrder_del() > 0 || item.getHandler().equals("8")) {
            ...
            view_pingjia.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View view) {
                    OrderUtils.getInstance().order_delete_post(item.getOrder_id() + "");   OrderUtils.getInstance().setOnOrderDellListener(new OrderUtils.OnOrderDelListener() {
                        @Override
    ...
    public View addView(String title, boolean isBga, OrderDaishouhuoAdapter.MyHolder helper) {
        LinearLayout mFooterLayout = helper.itemView.findViewById(R.id.ri_daishouhuo_item_container);
