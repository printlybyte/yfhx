 ...
 * ============================================
 **/
public class MeRedPackFragmentNot extends BaseFragment {
    private RecyclerView mIncludeRecyclerview;
    private SmartRefreshLayout mRefreshLayout;
    private MultipleStatusView mMultipleStatusView;
    private int mPage = 1;
    public static MeRedPackFragmentNot newInstance() {
        Bundle args = new Bundle();
        MeRedPackFragmentNot generalPurposeFragment = new MeRedPackFragmentNot();
        generalPurposeFragment.setArguments(args);
        return generalPurposeFragment;
    }
    @Override
    protected int getContentLayoutId() {
        return R.layout.fragment_readpack_not;
    }
                ...
                .setOnCallBackResponse(new CallBackResponseListener() {
                    ...
                    public void setOnCallBackResponseSuccess(String response) {
                        ...
                        if (beanx.getStatus().equals("success")) {
                            ...
                            if (bean_e != null && bean_e.getData().getUnable().size() > 0) {
                                if (isLoading) {
                                    meRedPackAdapter.addData(bean_e.getData().getUnable());
                                } else {
                                    setAdapter(bean_e);
                                }
                            } else {
    ...
    private MeRedPackNotAdapter meRedPackAdapter;

    private void setAdapter(SaveCardNotFragmentBean bean) {
        meRedPackAdapter = new MeRedPackNotAdapter(R.layout.ri_me_redpack_item, bean.getData().getUnable());
        ...
        mIncludeRecyclerview.setAdapter(meRedPackAdapter);
        meRedPackAdapter.openLoadAnimation();
    }
