...
import com.yinfeng.yfhx.adapter.order.OrdeAllAdapter;
...
public class OrderAllFragment extends BaseFragment {
    ...
    private OrdeAllAdapter ordeAllAdapter;

    public static OrderAllFragment newInstance() {
        Bundle args = new Bundle();
        OrderAllFragment generalPurposeFragment = new OrderAllFragment();
...
//        mIncludeRecyclerview.setNestedScrollingEnabled(false);
        ordeAllAdapter = new OrdeAllAdapter(R.layout.ri_all_item, bean.getData().getList());
        ordeAllAdapter.openLoadAnimation();
        mIncludeRecyclerview.setAdapter(ordeAllAdapter);
        ordeAllAdapter.setOnItemChildClickListener(onItemChildClickListener);
                    ...
                    public void setOnCallBackResponseSuccess(String response) {
                        ...
                        if (beanx.getStatus().equals("success")) {
                            ...
                            if (bean != null && bean.getData().getList().size() > 0) {
                                if (isLoading) {
                                    ordeAllAdapter.addData(bean.getData().getList());
                                } else {
                                    setAdapter(bean);
                                }
                            } else {
