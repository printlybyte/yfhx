...
import com.yinfeng.yfhx.adapter.me.MeSaveCardNotAdapter;
 ...
 * ============================================
 **/
public class SaveCardNotFragment extends BaseFragment {
    private RecyclerView mIncludeRecyclerview;
    private SmartRefreshLayout mRefreshLayout;
    private MultipleStatusView mMultipleStatusView;
    private int mPage = 1;

    public static SaveCardNotFragment newInstance() {
        Bundle args = new Bundle();
        SaveCardNotFragment generalPurposeFragment = new SaveCardNotFragment();
                    ...
                    public void setOnCallBackResponseSuccess(String response) {
                        ...
                        if (beanx.getStatus().equals("success")) {
                            ...
                            if (bean_e != null && bean_e.getData().getUnable().size() > 0) {
                                if (isLoading) {
                                    meInvoiceAdapter.addData(bean_e.getData().getUnable());
                                } else {
                                    setAdapter(bean_e);
                                }
                            } else {
    ...
    private MeSaveCardNotAdapter meInvoiceAdapter;
    private void setAdapter(SaveCardNotFragmentBean bean) {
          meInvoiceAdapter = new MeSaveCardNotAdapter(R.layout.ri_me_save_card_not_item, bean.getData().getUnable());
